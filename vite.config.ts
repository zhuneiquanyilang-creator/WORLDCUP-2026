...
async function runPeriodicCatchup(apiKey: string) {
  ...
  const updated = await withWriteLock<number>(async () => {
    ...
    for (const fx of fdMatches) {
      ...
      // FD の score フィールドの意味:
      //   regularTime = 90 分終了時 (信頼できる)、extraTime = ET 加算得点
      //   fullTime = regularTime + extraTime + PK 得点数 (PK 込みで返る)
      //   penalties = FD 独自集計だが値が誤る事例あり (m087 で 4-4 と誤送)
      // 表示スコア = regularTime + extraTime、PK 得点 = fullTime - regularTime - extraTime
      // 古い FD 応答で regularTime が無ければ旧ロジックにフォールバック。
