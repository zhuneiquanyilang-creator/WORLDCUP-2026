...
| 2 | **トーナメント表** (Bracket) | R32〜決勝のブラケットを左右対称で表示。グループ確定 / KO 勝者で自動反映 |
| 3 | **日程・結果** (Schedule) | 全試合の一覧。ステージ・ステータス・グループ・試合日でフィルタ可。カードクリックで詳細 |
| 4 | **スタッツ** (Stats) | 大会全体の個人記録（得点ランキング、アシストランキング） |
| 5 | **過去の大会** (Past) | 1930〜2022年の歴代W杯一覧（新しい順）。クリックで結果・各賞の詳細 |
| 6 | **コラム** (Columns) | 大会の見どころや本サイトの使い方などの読み物。クリックで本文 |
| 7 | **レギュレーション** (Regulations) | 大会公式ルール（出場国数・トーナメント方式・グループ順位決定方法・フェアプレーポイント等）を 1 ページにまとめて表示 |
| 8 | **表彰** (Awards) | 2026 大会の最終順位（1〜4位）と個人賞（MVP / シルバーボール / ブロンズボール / 得点王 / 最優秀GK / 最優秀若手選手 / ベストゴール） |

※ 元は「日程」と「試合結果」を分けていたが、機能重複のため `/schedule` に統合。`/matches` は `/schedule` へリダイレクト。試合詳細は引き続き `/matches/:id`。
※ トーナメント表は元々 `/schedule?view=bracket` の中に同居していたが、専用メニュー `/bracket` として独立。
